...
// default: activated
#if EMMK_CFG_PRINTF_SUPPORT_FLOAT == 1
#define PRINTF_SUPPORT_FLOAT
#endif

// support for the long long types (%llu or %p)
// default: activated
#if EMMK_CFG_PRINTF_SUPPORT_LONG_LONG == 1
...
// default: activated
#if EMMK_CFG_PRINTF_SUPPORT_PTRDIFF_T == 1
...
__attribute__((weak)) void ptf_putchar(char c) {
...
int klPtf_printf(const char* format, ...) {
    va_list va;
    
    va_start(va, format);
    
    char buffer[1];
    const int ret = __vsnprintf(_out_char, buffer, (size_t) -1, format, va);
...
int klPtf_sprintf(char* buffer, const char* format, ...) {
    va_list va;
...
int klPtf_snprintf(char* buffer, size_t count, const char* format, ...) {
    va_list va;
...
int klPtf_vsnprintf(char* buffer, size_t count, const char *format, va_list va) {
    return __vsnprintf(_out_buffer, buffer, count, format, va);
}


int klPtf_fctprintf(void (*out)(char character, void* arg), void* arg, const char* format, ...) {
    va_list va;
    va_start(va, format);
    
    const out_fct_wrap_type out_fct_wrap = { 
        out, arg 
    };
    
    const int ret = __vsnprintf(_out_fct, (char *) &out_fct_wrap, (size_t) -1, format, va);
...
int klPtf_vfctprintf(void (*out)(char character, void* arg), void* arg, const char* format, va_list va) {
    const out_fct_wrap_type out_fct_wrap = { 
        out, arg 
    };
