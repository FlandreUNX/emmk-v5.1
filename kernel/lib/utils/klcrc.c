...
static void _invertUint16(uint16_t *dBuf, uint16_t *srcBuf) {
    uint16_t tmp[4] = {0};
...
/**
 * @addtogroup CRC8 public functions
 * @note none
 */
...
uint8_t klib_crc8(uint8_t *msg, uint32_t len) {
    if (msg == NULL || len == 0) {
        return 0;
    }
...
// pre must be 0 if first call
uint8_t klib_crc8_inc(uint8_t crc8_in, uint8_t *msg, uint32_t len) {
    if (msg == NULL || len == 0) {
        return 0;
    }
    
	uint8_t crc8 = 0xFF ^ crc8_in;
...
uint8_t kcrc8_at(uint8_t *message, uint32_t length) {
...
uint8_t kcrc8_inc(uint8_t _crc8, uint8_t *message, uint32_t length) {
    if (message == NULL || length == 0) {
        return 0;
    }
    
	uint8_t crc8 = 0xFF ^ _crc8;
    
	for (uint32_t i = 0; i < length; i++) {
		crc8 = CRC8_TABLE[crc8 ^ message[i]];
    }
...
/**
 * @addtogroup CRC16 public functions
 * @note none
 */
...
uint16_t klib_crc16_f(const uint8_t *msg, uint16_t len) {
    if (msg == NULL || len == 0) {
        return 0;
    }
...
uint16_t klib_crc16(uint8_t *msg, uint32_t len) {
    if (msg == NULL || len == 0) {
        return 0;
    }
    ...
    while (len--) {
        index = crcLO ^ *(msg++);
        crcLO = (uint8_t) (crcHI ^ CRC16_TABLE_HI[index]);
        crcHI = CRC16_TABLE_LO[index];
    }
...
uint16_t klCRC16_modbus(const uint8_t *msg, uint16_t len) {
    uint16_t wCRCin = 0xFFFF;
    uint16_t wCPoly = 0x8005;

    _invertUint16(&wCPoly, &wCPoly);
...
/**
 * @addtogroup CRC32 public functions
 * @note none
 */
...
uint32_t klib_crc32(uint32_t crc32, uint8_t *msg, uint32_t len) {
#define INIT  0xFFFFFFFF
#define XOROT 0xFFFFFFFF

    while (len --) {
        crc32 = CRC32_TABLE[(crc32 ^ *msg++) & 0xFF] ^ (crc32 >> 8);
    }
