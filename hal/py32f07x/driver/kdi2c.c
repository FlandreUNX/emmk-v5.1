...
static inline int32_t halBlockDmaTxc(kdi2c_t *kd) {
    ...
    qSTimer_Set(&tmcd, 1000);
    while (!dma_flag_get(kd->_config.dma.dmaTx, kd->_config.dma.channelTx, DMA_FLAG_FTF) 
            && (!qSTimer_Expired(&tmcd))) {
        threadYield();
    }
...
static inline int32_t halBlockDmaRxc(kdi2c_t *kd) {
    ...
    qSTimer_Set(&tmcd, 1000);
    while (!dma_flag_get(kd->_config.dma.dmaRx, kd->_config.dma.channelRx, DMA_FLAG_FTF) 
            && (!qSTimer_Expired(&tmcd))) {
        threadYield();
    }
...
int32_t kdi2c_transmit(kdi2c_t *kd, uint8_t addr,
    ...
    if (kd->_instance.i2c != NULL) {   
        ...
        if (kd->_config.dma.channelTx != 0xFF) {
            i2c_dma_config(kd->_instance.i2c, I2C_DMA_ON);
            dma_interrupt_flag_clear(kd->_config.dma.dmaTx, 
                kd->_config.dma.channelTx, 
                DMA_INT_FLAG_G | DMA_INT_FLAG_FTF | DMA_INT_FLAG_ERR
            );
            DMA_CHCTL(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) &= ~DMA_CHXCTL_CHEN;
            DMA_CHCNT(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) = (wsize & DMA_CHANNEL_CNT_MASK);
            DMA_CHMADDR(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) = (uint32_t) wbuf;
            DMA_CHCTL(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) |= DMA_CHXCTL_CHEN;
        ...
        if (kd->_config.dma.channelRx != 0xFF) {
            if (rsize < 2) {
               goto l_blockRead;
                ...
                i2c_dma_config(kd->_instance.i2c, I2C_DMA_ON);
                dma_interrupt_flag_clear(kd->_config.dma.dmaRx, 
                    kd->_config.dma.channelRx, 
                    DMA_INT_FLAG_G | DMA_INT_FLAG_FTF | DMA_INT_FLAG_ERR
                );
                DMA_CHCTL(kd->_config.dma.dmaRx, kd->_config.dma.channelRx) &= ~DMA_CHXCTL_CHEN;
                DMA_CHCNT(kd->_config.dma.dmaRx, kd->_config.dma.channelRx) = (rsize & DMA_CHANNEL_CNT_MASK);
                DMA_CHMADDR(kd->_config.dma.dmaRx, kd->_config.dma.channelRx) = (uint32_t) rbuf;
...
l_exit:
    if (kd->_instance.i2c != NULL) {
        i2c_dma_config(kd->_instance.i2c, I2C_DMA_OFF);
        i2c_ack_config(kd->_instance.i2c, I2C_ACK_ENABLE);
        i2c_ackpos_config(kd->_instance.i2c, I2C_ACKPOS_CURRENT);
    }
...
int32_t kdi2c_regWrite(kdi2c_t *kd, uint8_t addr,
    ...
    if (kd->_instance.i2c != NULL) {    
        ...
        if (kd->_config.dma.channelTx != 0xFF) {
            ...
            DMA_CHCTL(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) &= ~DMA_CHXCTL_CHEN;
            DMA_CHCNT(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) = (len & DMA_CHANNEL_CNT_MASK);
            DMA_CHMADDR(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) = (uint32_t) data;
            DMA_CHCTL(kd->_config.dma.dmaTx, kd->_config.dma.channelTx) |= DMA_CHXCTL_CHEN;
            if (halBlockDmaTxc(kd) != 0) {
                dma_interrupt_flag_clear(kd->_config.dma.dmaTx, 
                    kd->_config.dma.channelTx, 
