...
static void _updateFreq(kdspi_t *kd, uint32_t speed) {
    ...
    switch ((uint32_t) kd->_config.spiConfig.spi) {
#ifdef SPI1
         case (uint32_t) SPI1:
    ...
    if (SPI_CTL0(kd->_config.spiConfig.spi) & (uint32_t) SPI_CTL0_SPIEN) {
        spi_disable(kd->_config.spiConfig.spi);
        SPI_CTL0(kd->_config.spiConfig.spi) &= ~SPI_PSC_256;
        SPI_CTL0(kd->_config.spiConfig.spi) |= CTL0_PSC((val));
        spi_enable(kd->_config.spiConfig.spi);
    } else {
        SPI_CTL0(kd->_config.spiConfig.spi) &= ~SPI_PSC_256;
        SPI_CTL0(kd->_config.spiConfig.spi) |= CTL0_PSC((val));
    }
...
int32_t kdspi_init(kdspi_t *kd) {
    ...
    spi_init(kd->_config.spiConfig.spi, &kd->_config.spiConfig.init);
    if (kd->_config.spiConfig.defualtSpeed != 0) {
        _updateFreq(kd, kd->_config.spiConfig.defualtSpeed);
    }
    
    if (kd->_config.dmaConfig.channelTx != 0xFF) {
        spi_dma_enable(kd->_config.spiConfig.spi, SPI_DMA_TRANSMIT);
        dma_init(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx, &kd->_config.dmaConfig.initTx);
        dma_circulation_disable(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx);
        dma_memory_to_memory_disable(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx);
        dma_interrupt_enable(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx, DMA_INT_FTF);
    }
    
    if (kd->_config.dmaConfig.channelRx != 0xFF) {
        spi_dma_enable(kd->_config.spiConfig.spi, SPI_DMA_RECEIVE);
        dma_init(kd->_config.dmaConfig.dmaRx, kd->_config.dmaConfig.channelRx, &kd->_config.dmaConfig.initRx);
        dma_circulation_disable(kd->_config.dmaConfig.dmaRx, kd->_config.dmaConfig.channelRx);
        dma_memory_to_memory_disable(kd->_config.dmaConfig.dmaRx, kd->_config.dmaConfig.channelRx);
    }
...
int32_t kdspi_finalize(kdspi_t *kd) {
    ...
    if (kd->_config.dmaConfig.channelTx != 0xFF) {
        spi_dma_disable(kd->_config.spiConfig.spi, SPI_DMA_TRANSMIT);
        dma_interrupt_disable(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx, DMA_INT_FTF);
        dma_deinit(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx);
    }
    
    if (kd->_config.dmaConfig.channelRx != 0xFF) {
        spi_dma_disable(kd->_config.spiConfig.spi, SPI_DMA_RECEIVE);
        dma_deinit(kd->_config.dmaConfig.dmaRx, kd->_config.dmaConfig.channelRx);
    }
            
    spi_i2s_deinit(kd->_config.spiConfig.spi);
...
int32_t kdspi_powerDown(kdspi_t *kd) {
    ...
    if (kd->_config.dmaConfig.channelTx != 0xFF) {
        dma_interrupt_disable(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx, DMA_INT_FTF);
        dma_channel_disable(kd->_config.dmaConfig.dmaTx, kd->_config.dmaConfig.channelTx);
    }
    
    if (kd->_config.dmaConfig.channelRx != 0xFF) {
        dma_channel_disable(kd->_config.dmaConfig.dmaRx, kd->_config.dmaConfig.channelRx);
    }
...
int32_t kdspi_transmit(kdspi_t *kd, uint8_t *wbuf, uint8_t *rbuf, uint16_t len, uint32_t timeout) {
    ...
    if (timeout < 0xFFFFFFFF
        ...
        dma_transfer_number_config(kd->_config.dma.dmaRx, kd->_config.dma.channelRx, len);
        dma_memory_address_config(kd->_config.dma.dmaRx, kd->_config.dma.channelRx, (uint32_t) rbuf);
        ...
        dma_transfer_number_config(kd->_config.dma.dmaTx, kd->_config.dma.channelTx, len);
        dma_memory_address_config(kd->_config.dma.dmaTx, kd->_config.dma.channelTx, (uint32_t) wbuf);
        ...
        for (register uint32_t i = 0; i < len; i++) {
            while (!(SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_TBE));
            SPI_DATA(kd->_config.spiConfig.spi) = (uint32_t) wbuf[i];

            while (!(SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_RBNE));
            rbuf[i] = SPI_DATA(kd->_config.spiConfig.spi);
        }
        
        while ((SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_TRANS));
...
int32_t kdspi_sendData(kdspi_t *kd, uint8_t *data, uint16_t len, uint32_t timeout) {
    ...
    if (timeout < 0xFFFFFFFF
        ...
        for (register uint32_t i = 0; i < len; i++) {
            while (!(SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_TBE));
            SPI_DATA(kd->_config.spiConfig.spi) = (uint32_t) data[i];
            
            while (!(SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_RBNE));
            SPI_DATA(kd->_config.spiConfig.spi);
        }
...
int32_t kdspi_recvData(kdspi_t *kd, uint8_t *data, uint16_t len, uint32_t timeout) {
    ...
    if (timeout < 0xFFFFFFFF
        ...
        dma_channel_disable(kd->_config.dma.dmaRx, kd->_config.dma.channelRx);
        dma_transfer_number_config(kd->_config.dma.dmaRx, kd->_config.dma.channelRx, len);
        dma_memory_address_config(kd->_config.dma.dmaRx, kd->_config.dma.channelRx, (uint32_t) data);
        dma_channel_enable(kd->_config.dma.dmaRx, kd->_config.dma.channelRx);
        ...
        if (kd->_config.dma.channelTx != 0xFF) {
            ...
            for (register uint32_t i = 0; i < len; i++) {
                while (!(SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_TBE));
                SPI_DATA(kd->_config.spiConfig.spi) = (uint32_t) 0xFF;
            }
        }
        ...
        for (register uint32_t i = 0; i < len; i++) {
            ...
            while (!(SPI_STAT(kd->_config.spiConfig.spi) & SPI_FLAG_RBNE));
            data[i] = SPI_DATA(kd->_config.spiConfig.spi);
        }
