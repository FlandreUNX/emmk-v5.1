...
int32_t kdpwm_init(kdpwm_t *kd) {
    ...
    if (TIMER_PSC(kd->_host->timer) != 0 && TIMER_CAR(kd->_host->timer) != 0) {
        return 0;
    } 
    ...
    if (kd->_host->timer == TIM1_BASE 
            // || kd->_instance->timer == TIM15_BASE
            || kd->_host->timer == TIM16_BASE
            || kd->_host->timer == TIM17_BASE) {
