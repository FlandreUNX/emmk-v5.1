...
#ifndef _KDSPI_SOC_H_
#define _KDSPI_SOC_H_
...
#define _KDSPI_PATH(x) "/sys/bus/spi/SPI"#x
    
#define KDSPI_MODULE(_number)       _number
#define _KDSPI_MODULE(_number)      SPI##_number
    
#define KDSPI_DMA_MODULE(_number)   _number
#define _KDSPI_DMA_MODULE(_number)  DMA##_number
...
#define KDSPI_ACTIVE_RX_DMA_HANDLE(x, _action) \
    dma_interrupt_flag_clearx._config.dma.dmaRx, x._config.dma.channelRx, DMA_INT_FLAG_G | DMA_INT_FLAG_FTF | DMA_INT_FLAG_ERR); \
    dma_channel_disable(x._config.dma.dmaRx, x._config.dma.channelRx); \
    if (_action) { \
        x._va->flag.isSendCompleted = 1; \
        x._va->flag.isRecvCompleted = 1; \
...
#define KDSPI_DEFINE(_moduleNumber, _name, \
    ...
    const kdspi_t _KDSPI_INAME(_name) = { \
        ...
        ._config = { \
            ...
            .spiConfig = { \
                .spi = (uint32_t) _KDSPI_MODULE(_moduleNumber), \
            ...
            .dmaConfig = { \
                .dmaTx = (uint32_t) (_dmaChannelTx == 0xFF ? 0 : _KDSPI_DMA_MODULE(_dmaTx)), \
                .dmaRx = (uint32_t) (_dmaChannelRx == 0xFF ? 0 : _KDSPI_DMA_MODULE(_dmaRx)), \
