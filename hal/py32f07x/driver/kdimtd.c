...
extern const uint32_t _FlashTimmingParam[8];
...
static int32_t pageProgram(kdimtd_t *kd, uint32_t addr, uint8_t *data, uint16_t size) {
    ...
    do {
        ...
        uint32_t _len = wlen / 4;
        for (uint32_t i = 0; i < _len; i++) {
            *(__IO uint32_t *)(addr + (i * 4)) = ((uint32_t *) (data))[i]; 
            if (i == _len - 2) {
