...
void _gpio_modeConfig(uint32_t port, uint32_t pin, kdgpio_Mode_t mode) {
    gpio_init(port, mode, GPIO_OSPEED_MAX, pin);
}
...
void kdgpio_toggle(kdgpio_t *kd) {
    GPIO_OCTL(kd->_config.base.port) = GPIO_OCTL(kd->_config.base.port) ^ kd->_config.base.pin;
}
...
void kdgpio_irqEnable(kdgpio_t *kd, kdgpio_EventTrigger_t t, kdgpio_SignalEvent_t cbEvent) {
    ...
    gpio_exti_source_select(kd->_config.base.port, kd->_config.extiConfig.pinSource);
}
