...
typedef enum {
    KDGPIO_MODE_INPUT = GPIO_MODE_IN_FLOATING,
    KDGPIO_MODE_OUTPUT_PP = GPIO_MODE_OUT_PP,
    KDGPIO_MODE_OUTPUT_OD = GPIO_MODE_OUT_OD,
    
    KDGPIO_MODE_AF_PP = GPIO_MODE_AF_PP,
    KDGPIO_MODE_AF_OD = GPIO_MODE_AF_OD,
    
    KDGPIO_MODE_AIN = GPIO_MODE_AIN,
} kdgpio_Mode_t;
...
#define _KDGPIO_EXTI_PORT_SOURCE(_port)     GPIO_PORT_SOURCE_GPIO##_port
#define _KDGPIO_EXTI_PIN_SOURCE(_pin)       GPIO_PIN_SOURCE_##_pin
#define _KDGPIO_EXTI_MODE(_pin)             EXTI_##_pin
...
#define KDGPIO_DEFINE(_port, _pin, _name, \
    ...
    const kdgpio_t _KDGPIO_INAME(_name) = { \
        ...
        ._config = { \
            ...
            .extiConfig = { \
                .portSource = _KDGPIO_EXTI_PORT_SOURCE(_port), \
                .pinSource = _KDGPIO_EXTI_PIN_SOURCE(_pin), \
                .eLine = _KDGPIO_EXTI_MODE(_pin), \
            }, \
        }, \
    };
...
extern void _gpio_modeConfig(uint32_t port, uint32_t pin, kdgpio_Mode_t mode);
extern void _gpio_pullConfig(uint32_t port, uint32_t pin, kdgpio_PullResistor_t pull);
extern void _gpio_output(uint32_t port, uint32_t pin, bool set);
extern void _gpio_afConfig(uint32_t port, uint32_t number, uint32_t af);
