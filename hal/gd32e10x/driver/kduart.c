...
int32_t kduart_init(kduart_t *kd) {
    ...
    usart_word_length_set(kd->_config.uart.uart, USART_WL_8BIT);
...
int32_t kduart_hasRecvData(kduart_t *kd) {
    ...
    if (flag > 0) {
        if (kd->buffer.recvLwrb != NULL) {
            ...
            return (int32_t) kd->buffer.recvBufferSize - (int32_t) dma_transfer_number_get(kd->_config.dma.dmaRx, kd->_config.dma.channelRx);
        }
    }
    return 0;
}
