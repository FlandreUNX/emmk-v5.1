...
static int32_t sectorErase(kdimtd_t *kd, uint32_t addr) {
    FLASH_UnlockPages(addr, addr + kd->_instance.blockSize - 1);
    
//    uint16_t  CR1BAK, CR2BAK;
//    while (CW_FLASH->ISR & bv5);
//    CW_FLASH->ICR = 0x00;
//    CR2BAK = CW_FLASH->CR2;
//    CR1BAK = CW_FLASH->CR1;
...
//    CW_FLASH->CR2 = 0x5A5A0000 | CR2BAK;
    FLASH_ErasePages(addr, addr + kd->_instance.blockSize - 1);
    
    FLASH_ClearITPendingBit(FLASH_IT_PAGELOCK | FLASH_IT_PC | FLASH_IT_PROG);
    FLASH_LockPages(addr, addr + kd->_instance.blockSize - 1);
