...
int32_t kdsaradc_init(kdsaradc_t *kd) {
    ...
    if (kd->_config.host->init.adc.isContinueMode) {
        ...
        adc_discontinuous_mode_config(ADC_ROUTINE_CHANNEL, kd->_config.host->init.adc.channelNumber);
    }
    adc_special_function_config(ADC_SCAN_MODE, ENABLE);
    adc_data_alignment_config(ADC_DATAALIGN_RIGHT);
    
    adc_channel_length_config(ADC_ROUTINE_CHANNEL, kd->_config.host->init.adc.channelNumber);
    uint8_t index = 0;
    for (uint8_t i = 0; i < KDSARADC_CHANNEL_RESERVED; i++) {
        if (kd->_config.host->init.adc.enableChannels & (1u << i)) {
            adc_routine_channel_config(index, i, kd->_config.host->init.adc.channelSampleSpeed[index]);
            index++;
        }
    }
