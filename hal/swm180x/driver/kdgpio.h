...
typedef enum {
    KDGPIO_TRIGGER_NONE,
    KDGPIO_TRIGGER_RISING = 0x80 | EXTI_FALL_EDGE,
    KDGPIO_TRIGGER_FALLING = 0x80 | EXTI_RISE_EDGE,
    KDGPIO_TRIGGER_RISING_FALLING = 0x80 | EXTI_BOTH_EDGE,
    KDGPIO_TRIGGER_LOW = 0x80 | EXTI_LOW_LEVEL,
    KDGPIO_TRIGGER_HIGH = 0x80 | EXTI_HIGH_LEVEL,
...
#define _KDGPIO_PORT(_port)  PORT##_port

#define _KDGPIO_GPIO(_port)  GPIO##_port

#define KDGPIO_PIN(_pin)     _pin
#define _KDGPIO_PIN(_pin)    PIN##_pin
...
#define KDGPIO_DEFINE(_port, _pin, _name, \
    ...
    const kdgpio_t _KDGPIO_INAME(_name) = { \
        ...
        ._config = { \
            .base = { \
                .gpio = _KDGPIO_GPIO(_port), \
                .port = _KDGPIO_PORT(_port), \
                .number = _pin, \
                .pin = 1 << _KDGPIO_PIN(_pin), \
            }, \
            .downCfg = { \
                .mode = _dmode, \
                .pull = _dpull, \
...
extern void _gpio_modeConfig(PORT_TypeDef *port, GPIO_TypeDef *gpio, uint32_t pin, kdgpio_Mode_t mode);
extern void _gpio_pullConfig(PORT_TypeDef *port, uint32_t pin, kdgpio_PullResistor_t pull);
