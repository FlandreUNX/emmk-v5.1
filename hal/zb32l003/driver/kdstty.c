...
static inline void pinSet(kdstty_Pin_t *pin, uint32_t set) {
    ...
    if (set) {
        pin->gpio->_config.base.port->ODSET = pin->gpio->_config.base.pin;
    } else {
        pin->gpio->_config.base.port->ODCLR = pin->gpio->_config.base.pin;
    }
}
...
int32_t kdstty_init(kdstty_t *kd) {
    ...
    if (emmkDriver_initRefsCountUp(&kd->_host->_va->initRefs) == 0) {
        ...
        if (kd->_host->_config.tim.isLp == 0) {
            // 24Mhz = 0.041us
            arr = UINT32_MAX - ((uint32_t) (arr / 0.041f));
            kd->_host->_config.tim.tim->CR |= 
                (0x01u << 6u)
                | (0x01u << 5u)
                | (0x01u << 4);
                // | (0x07u << 0);
            kd->_host->_config.tim.tim->INTCLR = 0x01;
            kd->_host->_config.tim.tim->BGLOAD = arr;
            kd->_host->_config.tim.tim->LOAD = kd->_host->_config.tim.tim->BGLOAD;
        } else {
            // 24Mhz = 0.041us
            // MAX-Count = 2.686ms
            // 1byte(10bit)us
            ...
            kd->_host->_config.tim.lptim->CR = 0;
            kd->_host->_config.tim.lptim->CR =
                (0x01u << 9u)
                | (0x10 << 4)
                | KLBIT(8)
                | (0x01u << 1u)
            ...
            kd->_host->_config.tim.lptim->INTCLR = 0x01;
            kd->_host->_config.tim.lptim->LOAD = kd->_host->_config.tim.lptim->BGLOAD = arr;
        }
    }
...
int32_t kdstty_finalize(kdstty_t *kd) {
    ...
    if (emmkDriver_initRefsCountDown(&kd->_host->_va->initRefs) == 0) {
        if (kd->_host->_config.tim.isLp == 0) {
            kd->_host->_config.tim.tim->CR &= ~(0x01u << 7u);
        } else {
            kd->_host->_config.tim.lptim->CR &= ~(0x01u << 0u);
        }
...
int32_t kdstty_powerUp(kdstty_t *kd) {
    ...
    if (emmkDriver_initRefsCountUp(&kd->_host->_va->initRefsPower) == 0) {
        ...
        if (kd->_host->_config.tim.isLp == 0) {
            kd->_host->_config.tim.tim->INTCLR |= 0x01; 
            kd->_host->_config.tim.tim->CR &= ~(0x01 << 7); 
            kd->_host->_config.tim.tim->LOAD = kd->_host->_config.tim.tim->BGLOAD; 
            kd->_host->_config.tim.tim->CR |= (0x01 << 7); 
        } else {
            kd->_host->_config.tim.lptim->INTCLR |= 0x01; 
            kd->_host->_config.tim.lptim->CR &= ~(0x01 << 0); 
            kd->_host->_config.tim.lptim->LOAD = kd->_host->_config.tim.lptim->BGLOAD; 
            kd->_host->_config.tim.lptim->CR |= (0x01 << 0); 
        }
    }
