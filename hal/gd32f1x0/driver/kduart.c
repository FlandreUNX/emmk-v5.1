...
static void updateBaudRate(kduart_t *kd, uint32_t bd, uint32_t to) {
    ...
    if (kd->_config.timer.timer != NULL) {
        ...
        usart_receiver_timeout_config(kd->_config.uart.uart, to);
    }
}
...
int32_t kduart_init(kduart_t *kd) {
    ...
    usart_word_length_set(kd->_config.uart.uart, kd->_config.uart.init.wlen);
    ...
    if (kd->_config.timer.timer == NULL) {
        usart_receiver_timeout_config(kd->_config.uart.uart, kd->_config.rto * (11));
    ...
    if (kd->_config.timer.timer != NULL) {
        ...
        if (kd->_config.dma.channelRx != 0xFF) {
            ...
            usart_dma_receive_config(kd->_config.uart.uart, USART_DENR_ENABLE);
        }
        ...
        if (kd->_config.dma.channelTx != 0xFF) {
            ...
            usart_dma_transmit_config(kd->_config.uart.uart, USART_DENT_ENABLE);
        }
    }
...
int32_t kduart_powerUp(kduart_t *kd) {
    ...
    if (kd->_config.dma.channelRx != 0xFF) {
        dma_channel_disable(kd->_config.dma.channelRx);
        dma_transfer_number_config(kd->_config.dma.channelRx, kd->buffer.recvBufferSize);
        dma_memory_address_config(kd->_config.dma.channelRx, (uint32_t) kd->buffer.recvBuffer);
    ...
    usart_interrupt_flag_clear(kd->_config.uart.uart, USART_INT_FLAG_TBE);
...
int32_t kduart_sends(kduart_t *kd, const void *data, uint32_t size, uint32_t timeout) {
...
#if EMMK_FULL_RTOS_SUPPORT == EMMK_FULL_RTOS_RTX5
    uint32_t flag;
    flag = osEventFlagsWait(kd->_va->flag, UART_FLAG_SEND_COMPLETE, osFlagsWaitAll | osFlagsNoClear, timeout);
    if ((flag & 0x80000000) || ((flag & (UART_FLAG_SEND_COMPLETE)) != (UART_FLAG_SEND_COMPLETE))) {
        return -1;
    }
    osEventFlagsClear(kd->_va->flag, UART_FLAG_SEND_COMPLETE);
#else
    ...
    if (kd->_config.dma.channelTx != 0xFF) {
        dma_channel_disable(kd->_config.dma.channelTx);
        dma_transfer_number_config(kd->_config.dma.channelTx, size);
...
int32_t kduart_recvs(kduart_t *kd, void *data, uint32_t expect_size, uint32_t *recv_size, uint32_t timeout) {
    ...
    if (timeout != 0) {
#if EMMK_FULL_RTOS_SUPPORT == EMMK_FULL_RTOS_RTX5
        uint32_t flag = osEventFlagsWait(kd->_va->flag, UART_FLAG_RECV_COMPLETE, osFlagsWaitAll | osFlagsNoClear, timeout);
    ...
    if (kd->_config.timer.timer == NULL && kd->_config.dma.channelRx != 0xFF) {
        ...
        rcv_len = kd->buffer.recvBufferSize - dma_transfer_number_get(kd->_config.dma.channelRx);
...
int32_t kduart_flush(kduart_t *kd) {
    usart_interrupt_disable(kd->_config.uart.uart, USART_INT_TBE);
    usart_interrupt_disable(kd->_config.uart.uart, USART_INT_TC);
