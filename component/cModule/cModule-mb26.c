...
/// @-1=Disable
#if CONFIG_CMODULE_INSTANCE_X_ENABLE == 54
#define CONFIG_CMODULE_INSTANCE_TAG_ID                    52
#else
...
static int32_t queryIMEI(void) {
    ...
    if (rilat_writeMultiline(&mModuleInstance.rilat.instance, AT_CGSN, "+CGSN:", &response, 1000) != 0 || response == NULL || response->success == 0) {
        goto _l_retryExit;
    }
    ...
    klib_str_skipWhiteSpace(&line);
    
    strncpy(mModuleInstance.modemInfo.imei, line, 15);
...
static int32_t readIccid(void) {
    static const char AT_CSQ[] = "AT+ECICCID";

    int8_t rc = -1;
    Rilat_AtResponse_t *response = NULL;

    if (rilat_writeSingleline(&mModuleInstance.rilat.instance, AT_CSQ, "+ECICCID:", &response, 1000) != 0 ||
        response == NULL || response->success == 0) {
        goto _l_retryExit;
    }
...
static int32_t readCell(void) {
    static const char AT[] = "AT+ECBCINFO=0,8,0,1,0";
    Rilat_AtResponse_t *response = NULL;

    if (rilat_writeSingleline(&mModuleInstance.rilat.instance, AT, "+ECBCINFOSC:", &response, 1000) != 0 ||
        response == NULL || response->success == 0) {
        goto _l_retryExit;
    }
    ...
    klAttoken_skip(&line);  // earfcn
    int32_t i32 = 0;
    if (klAttoken_getNextInt(&line, &i32) == 0) {
       mModuleInstance.modemInfo.sei.pci = i32;
    } else {
        goto _l_retryExit;
    }
    if (klAttoken_getNextInt(&line, &i32) == 0) {
       mModuleInstance.modemInfo.sei.rsrp = i32;
    } else {
        goto _l_retryExit;
    }
    if (klAttoken_getNextInt(&line, &i32) == 0) {
       mModuleInstance.modemInfo.sei.sinr = i32;
    } else {
        goto _l_retryExit;
    }
    char *cellId = NULL;
    if (klAttoken_getNextString(&line, &cellId, &i32) == 0) {
        if (i32 > 0 && i32 <= 6) {
            mModuleInstance.modemInfo.sei.cellId = atoi(cellId);
        }
    } else {
        goto _l_retryExit;
    }
...
static void reboot(void) {
    static const char AT_RST[] = "AT+ECRST";
...
static int32_t lwm2mDelete(uint32_t commandTimeout) {
    rilat_writeLine(&mModuleInstance.rilat.instance, "AT+LWM2MDELETE", NULL, commandTimeout);
    return 0;
}
...
static int32_t lwm2mUpdate(uint32_t commandTimeout) {
    ...
    mModuleInstance.state.urcResponseFlags &= ~(CMODULE_URC_FLAG_STACK_UPDATE_SUCCESS | CMODULE_URC_FLAG_CONN_FAILED);
...
static void onLoop(void) {
    if (qSTimer_Expired(&mModuleInstance.aux.pollTimer)) {
        qSTimer_Set(&mModuleInstance.aux.pollTimer, 10000);
        
        if (lwm2mUpdate(10000) != 0) {
...
static int32_t onPtPackIdCustom(cModule_TransmitPackageInfo_t *info) {
    if (info->flag.requestId == TRANSMIT_PACK_REQ_ID_NTP_UPDATE) {
        checkNTP();
    }
    return 0;
}
...
cModule_Instance_t *cModule_getInstance_mb26(void) {
