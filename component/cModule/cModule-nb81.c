...
/// @-1=Disable
#if CONFIG_CMODULE_INSTANCE_X_ENABLE == 01
#define CONFIG_CMODULE_INSTANCE_TAG_ID                    01
#else
...
static void onLoop(void) {
    if (qSTimer_Expired(&mModuleInstance.aux.pollTimer)) {
        ...
        AT();
        if (checkNTP() != 0 || readSignal() <= -113 || lwm2mUpdate(30000) != 0) {
...
cModule_Instance_t *cModule_getInstance_nb81(void) {
